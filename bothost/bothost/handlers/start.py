...
HELP = (
    ...
    f"Подробности в Политике."
)


def render_terms(cfg: Config) -> str:
    contact = cfg.admin_contact_html()
    return (
        ...
        f"{e.PERSON_OK} <b>Контакт.</b> {contact}."
    )


def render_privacy(cfg: Config) -> str:
    contact = cfg.admin_contact_html()
    return (
        ...
        f"{e.TRASH} <b>Удаление.</b> Напиши админу ({contact}) — "
        ...
        f"исправить их или потребовать удаления — {contact}."
    )
